...
function tapLove(id) {
  ...
  const loveIcon = document.querySelector(
    `#loveIcons .love-icon:nth-child(${id})`,
  );
  ...
  if (loveTaps.size === 4) {
    Swal.fire({
      ...
      typeWriterEffect(
        `Chúc ${userName} ngày 1/6 luôn luôn vui vẻ, hạnh phúc. Chúc bé iu mãi mãi là ebe của chòn. Moãi iu ebe nhó .... 💘`,
        "loveMsg",
        () => {
          const fromTag = document.createElement("div");
          fromTag.id = "fromTag";
          fromTag.textContent = "From: Chòn iu siu cấp đẹp trai của bé ❤️";
